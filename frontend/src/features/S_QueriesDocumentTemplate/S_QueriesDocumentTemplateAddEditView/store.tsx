...
import { getS_QueriesDocumentTemplate } from "api/S_QueriesDocumentTemplate";
import { createS_QueriesDocumentTemplate } from "api/S_QueriesDocumentTemplate";
import { updateS_QueriesDocumentTemplate } from "api/S_QueriesDocumentTemplate";
...
class NewStore {
  ...
  async onSaveClick(onSaved: (id: number) => void) {
    ...
    try {
      ...
      let response;
      if (this.id === 0) {
        response = await createS_QueriesDocumentTemplate(data);
      } else {
        response = await updateS_QueriesDocumentTemplate(data);
      }
  ...
  async doLoad(id: number) {
    ...
    this.loadS_QueriesDocumentTemplate(id);
  }

  loadS_QueriesDocumentTemplate = async (id: number) => {
    try {
      MainStore.changeLoader(true);
      const response = await getS_QueriesDocumentTemplate(id);
