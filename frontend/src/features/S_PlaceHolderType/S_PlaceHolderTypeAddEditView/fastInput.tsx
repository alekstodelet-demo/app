...
import storeList from "./../S_PlaceHolderTypeListView/store";
...
const FastInputView: FC<S_PlaceHolderTypeProps> = observer((props) => {
  ...
  useEffect(() => {
    if (props.idMain !== 0 && storeList.idMain !== props.idMain) {
      ...
      storeList.loadS_PlaceHolderTypes();
    }
  }, [props.idMain]);
  ...
  const columns = [
    {
                ...
                },
                {
                    field: 'queueNumber',
                    width: null, //or number from 1 to 12
                    headerName: translate("label:SmProjectTagListView.queueNumber"),
                },
  ...
  return (
    <Container>
      <Card component={Paper} elevation={5}>
        <CardContent>
          <Box id="S_PlaceHolderType_TitleName" sx={{ m: 1 }}>
            <h3>{translate("label:S_PlaceHolderTypeAddEditView.entityTitle")}</h3>
          </Box>
          <Divider />
                    ...
                    {storeList.isEdit === false && (
                      <>
                        ...
                        <IconButton
                          ...
                          onClick={() => storeList.deleteS_PlaceHolderType(entity.id)}
                        >
                          <DeleteIcon />
                        </IconButton>
                      </>
          ...
          {storeList.isEdit ? (
            ...
            <Grid item display={"flex"} justifyContent={"flex-end"} sx={{ mt: 2 }}>
              <CustomButton
                variant="contained"
                size="small"
                id="id_S_PlaceHolderTypeAddButton"
                onClick={() => {
                  storeList.setFastInputIsEdit(true);
                  store.doLoad(0);
                }}
